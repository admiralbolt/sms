...
from api.models import Artist, CarpenterRecord, CarpenterRun, Event, OpenMic, RawData, Venue
...
class Carpenter:
  ...
  def get_or_create_event(self, raw_data: RawData, api: EventApi, venue: Venue, artists: list[Artist]) -> Event:
    ...
    event_change_type, event_change_log, event = event_utils.create_or_update_event(
      venue=venue, raw_data=raw_data, artists=artists, **event_kwargs, event_api=api.api_name
    )
    CarpenterRecord.objects.create(
      carpenter_run=self.carpenter_run,
      api_name=api.api_name,
