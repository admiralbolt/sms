...
from api.constants import EventTypes, VenueTypes, get_all
from api.utils import search_utils, venue_utils
...
class OpenMicViewSet(viewsets.ModelViewSet):
  ...
  def get_queryset(self):
    mics = models.OpenMic.objects.order_by("day", "venue__name")
...
def get_events_on_day(request):
  ...
  return JsonResponse(serializers.EventSerializer(events, many=True).data, safe=False)
...
def search_events(request: HttpRequest):
  ...
  return JsonResponse(
    serializers.EventSerializer(search_utils.search_all_events(keyword, include_hidden=include_hidden), many=True).data, safe=False
  )
...
def search_artists(request: HttpRequest):
  ...
  return JsonResponse(serializers.ArtistSerializer(search_utils.search_all_artists(keyword), many=True).data, safe=False)
