...
class Command(BaseCommand):
  ...
  def handle(self, *args, **kwargs):
    if kwargs["truncate"]:
      Event.objects.filter(event_api=IngestionApis.EVENTBRITE).delete()
      venue_utils.clear_api_data(api_name=IngestionApis.EVENTBRITE)

    import_eventbrite_data(debug=kwargs["debug"])
